...
def inverse_expansion_fan_function(psi_target, gamma, mach_nozzle, tol=1e-10, maxiter=60):
    def dpsi_dphi(phi, mach_nozzle_var, gamma=1.4):
        vinv = inverse_prandtl_meyer(prandtl_meyer(mach_nozzle_var, gamma) + phi, gamma)
        return 1 + ( 1 + (gamma-1)*0.5*vinv**2 )/ ( vinv**2 - 1)

    def psi(phi, mach_nozzle_var, gamma=1.4):
        return phi-np.arcsin(1/(inverse_prandtl_meyer(prandtl_meyer(mach_nozzle_var, gamma) + phi, gamma)))

    phi = 0.0  # initial guess

    for _ in range(maxiter):

        f = psi(phi, mach_nozzle, gamma) - psi_target
        if abs(f) < tol:
            return phi
        df = dpsi_dphi(phi, mach_nozzle, gamma)
        if df == 0:
...
class column:
    ...
    def next_step(self, gamma, next_phi_edge, custom_nu_edge=0):
        ...
        for i in range(1, self.n_points-1):
            ...
            if (next_column.phi_array[0, i] + mu_p > np.pi/2) or ((next_column.phi_array[0, i] - mu_p < -np.pi/2)):
        ...
        if custom_nu_edge ==0:
            ...
            next_column.nu_array[0,-1] = custom_nu_edge
        ...
        Beta = 0.5*(self.phi_array[0, -1] - np.arcsin(1/inverse_prandtl_meyer(self.nu_array[0,-1], gamma)) + self.phi_array_2[0,-1] - np.arcsin(1/inverse_prandtl_meyer(self.nu_array_2[0,-1], gamma)))
        ...
        # Dx1 = np.cos(self.phi_array[0,-1]) * np.sin(Alp - Beta) * np.sqrt((self.x_array_2[0,-1] - self.x_array[0,-1])**2 + (self.y_array_2[0,-1] - self.y_array[0,-1])**2) / np.sin(Alp - self.phi_array[0, -1])
        # Dy1 = np.tan(self.phi_array[0,-1]) * Dx1
        # T = np.arctan((self.y_array[0,-1] + Dy1 - self.y_array_2[0,-1])/(self.x_array_2[0,-1] - Dx1 - self.x_array[0,-1]))
        # Dx2 = np.cos(next_column.phi_array[0,-1]) * np.sin(Alp-Beta) * np.sqrt((self.x_array_2[0,-1] - self.x_array[0,-1] -Dx1)**2 + (self.y_array_2[0,-1] - self.y_array[0,-1] -Dy1)**2) / np.sin(Alp - Beta - next_column.phi_array[0,-1] - T)
        # Dy2 = np.tan(next_column.phi_array[0,-1]) * Dx2
        # next_column.x_array[0, -1] = self.x_array[0,-1] + Dx1 + Dx2
        # next_column.y_array[0, -1] = self.y_array[0,-1] + Dy1 + Dy2
        R = (next_column.y_array[0,-2] - next_column.y_array[0,-3])/(next_column.x_array[0,-2] - next_column.x_array[0,-3])
        next_column.x_array[0,-1] = (1/(R-np.tan(next_column.phi_array[0,-1])))*(self.y_array[0,-1] - self.x_array[0,-1]*np.tan(next_column.phi_array[0,-1]) - next_column.y_array[0,-2] + next_column.x_array[0,-2]*R)
        next_column.y_array[0,-1] = self.y_array[0,-1] + np.tan(next_column.phi_array[0,-1]) * (next_column.x_array[0,-1] - self.x_array[0,-1])
        for i in range(self.n_points-1):
            ...
            if (next_column.phi_array_2[0, i] + mu_p > np.pi / 2) or ((next_column.phi_array_2[0, i] - mu_p < -np.pi / 2)):
                next_column.stop = True
            # if i != self.n_points - 2:
            #     if (next_column.y_array_2[0, i+1] < next_column.y_array_2[0, 1]) or (next_column.y_array_2[0, i+1] < 0):
            #         next_column.stop = True
...
while columns[0].stop == False and counter2 < 0*16:
    counter2 += 1
...
stopstop = False
while stopstop == False and counter < 500:
    counter += 1
